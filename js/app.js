            ...
            if(data.tanggal === today) { 
                if(data.status === "Selesai") { 
                    ...
                    if(!onlyBadge) { 
                        ...
                        // LOGIKA TOMBOL: Bedain buat Admin vs User Biasa
                        let actionBtn = "";
                        if (isAdminKitchen) {
                            actionBtn = `<button onclick="selesaikanPesananCafe('${id}')" class="w-full bg-green-500 hover:bg-green-600 text-white font-black py-2.5 rounded-lg text-xs transition active:scale-95 shadow-md shadow-green-500/30"><i class="fa-solid fa-check mr-2"></i> TANDAI SELESAI</button>`;
                        } else if (data.status === "Menunggu Dibuat") {
                            // Munculin tombol Batalkan kalo Dapur belum bikin
                            actionBtn = `<button onclick="batalkanPesananCafe('${id}')" class="w-full bg-red-100 hover:bg-red-200 text-red-600 font-black py-2.5 rounded-lg text-xs transition active:scale-95 border border-red-200"><i class="fa-solid fa-xmark mr-2"></i> BATALKAN PESANAN</button>`;
                        } else {
                            actionBtn = `<div class="text-center text-[10px] text-orange-500 font-bold bg-orange-50 py-2 rounded-lg border border-orange-100"><i class="fa-solid fa-fire-burner mr-1"></i> Sedang disiapkan dapur...</div>`;
                        }

                        list.innerHTML += `<div class="bg-white dark:bg-slate-800 p-4 rounded-xl border-l-4 border-orange-500 shadow-sm relative mb-3 fade-in"><div class="flex justify-between items-start mb-2 border-b border-gray-100 dark:border-slate-700 pb-2"><div><h4 class="font-bold text-sm text-slate-800 dark:text-white">${data.pemesan}</h4><p class="text-[9px] text-gray-400 uppercase font-bold">${data.divisi || 'Karyawan'} • <i class="fa-regular fa-clock"></i> ${data.waktu}</p></div><div class="text-right"><span class="bg-orange-100 text-orange-600 px-2 py-0.5 rounded text-[9px] font-black animate-pulse">${data.status}</span><p class="text-[10px] font-black text-orange-600 mt-1">Rp ${(data.total_harga || 0).toLocaleString('id-ID')}</p></div></div><div class="mb-4">${itemsHTML}</div>${actionBtn}</div>`; 
                    } 
                } 
            } 
        }); 
